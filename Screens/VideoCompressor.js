...
const VideoCompressor = ({ navigation }) => {
  ...
  const saveVideo = async () => {
    ...
    try {
      ...
      if (status !== 'granted') {
        triggerToast('Permission needed', 'Please grant storage access to save the video.', 'alert', 3000);
        return;
      }
      await MediaLibrary.saveToLibraryAsync(compressedUri);
      triggerToast('Saved', 'Compressed video saved to your gallery.', 'success', 3000);
    } catch (error) {
